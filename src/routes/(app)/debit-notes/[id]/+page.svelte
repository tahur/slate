<script lang="ts">
	...
	let { data } = $props();
	const { debitNote, vendor, org } = data;
</script>
		...
		<div class="flex items-center gap-4">
			...
			<div>
				<div class="flex items-center gap-3">
					...
					>
						{data.debitNote.debit_note_number}
					</h1>
					<StatusBadge status={data.debitNote.status} />
					<span
						class="text-slate-700"
						title="This debit note is posted and cannot be modified"
					>
						<Lock class="size-4" />
					</span>
				</div>
				<p class="text-xs text-slate-700 mt-0.5">
					Issued on {formatDate(data.debitNote.debit_note_date)}
				</p>
			</div>
		</div>
		<ButtonGroup.Root>
			...
			>
				...
				<div class="flex justify-between items-start">
					<div class="space-y-2">
						...
						{/if}
						{#if data.vendor?.billing_address}
							<p
					...
					<div class="text-right space-y-1">
						...
						<p class="font-mono text-[#111]">
							{formatDate(data.debitNote.debit_note_date)}
						</p>
					</div>
				</div>
							...
							<tr>
								<td class="px-4 py-4 text-[#111]">
									<p class="font-medium capitalize">
										{data.debitNote.reason}
									</p>
									{#if data.debitNote.notes}
										<p class="text-slate-800 mt-1">
											{data.debitNote.notes}
										</p>
									{/if}
								</td>
								<td
								...
								>
									{formatINR(data.debitNote.total)}
								</td>
							</tr>
						</tbody>
					</table>
					...
					<div class="w-64 space-y-3">
						...
						>
							...
							<span class="font-mono text-[#111]"
								>{formatINR(data.debitNote.balance)}</span
							>
						</div>
					</div>
				</div>
