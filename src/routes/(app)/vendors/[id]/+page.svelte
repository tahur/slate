        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                {#if data.vendor.company_name}
                    <p class="text-sm text-text-muted">{data.vendor.company_name}</p>
                {/if}
            </div>
        </div>
    ...
    <div class="flex-1 overflow-y-auto bg-surface-1">
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-surface-0 rounded-lg border border-border p-4">
                    <div class="flex items-center gap-2 text-text-muted mb-1">
                        ...
                        <span class="text-xs font-medium uppercase tracking-wider">Expenses</span>
                    </div>
                    <p class="text-xl font-bold text-text-strong">{data.summary.expenseCount}</p>
                </div>
                <div class="bg-surface-0 rounded-lg border border-border p-4">
                    <div class="flex items-center gap-2 text-text-muted mb-1">
            ...
            <div class="grid md:grid-cols-3 gap-4">
                ...
                <div class="bg-surface-0 rounded-lg border border-border p-4 space-y-3">
                    ...
                    {#if data.vendor.website}
                        <div class="flex items-center gap-2 text-sm">
                            ...
                            <a href={data.vendor.website} target="_blank" class="text-primary hover:underline">
                                {data.vendor.website}
                            </a>
                        </div>
                    {/if}
                ...
                <!-- GST Info Card -->
                <div class="bg-surface-0 rounded-lg border border-border p-4 space-y-3">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-text-muted">GST & Tax</h3>
                    <div class="flex items-center gap-2 text-sm">
                        <Building2 class="size-4 text-text-muted" />
                        <span class="capitalize">{data.vendor.gst_treatment || "Unregistered"}</span>
                    </div>
                    {#if data.vendor.gstin}
                        <div class="text-sm">
                            <span class="text-text-muted">GSTIN:</span>
                            <span class="font-mono ml-2">{data.vendor.gstin}</span>
                        </div>
                    {/if}
                    {#if data.vendor.pan}
                        <div class="text-sm">
                    ...
                    <div class="text-sm">
                        ...
                        <span class="ml-2">{getStateName(data.vendor.state_code)}</span>
                    </div>
                    {#if data.vendor.tds_applicable}
                        <div class="text-sm">
                            <span class="text-text-muted">TDS:</span>
                            <span class="ml-2">{data.vendor.tds_section || "Applicable"}</span>
                        </div>
                    {/if}
                </div>
                ...
                <div class="bg-surface-0 rounded-lg border border-border p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <Button variant="outline" size="sm" href="/expenses/new?vendor={data.vendor.id}" class="justify-start">
                    ...
                    {#if data.vendor.notes}
                        <div class="pt-2 border-t border-border">
                            <p class="text-xs text-text-muted">{data.vendor.notes}</p>
                        </div>
                    {/if}
                </div>
            </div>
                ...
                <div class="p-4">
                    {#if data.expenses.length === 0}
                        <div class="text-center py-12 text-text-muted">
                            ...
                            <Button href="/expenses/new?vendor={data.vendor.id}" class="mt-4">
                            ...
                            <tbody class="divide-y divide-border-subtle">
                                {#each data.expenses as expense}
                                    <tr class="hover:bg-surface-2/50">
                                        <td class="py-3 pr-4 text-text-muted">{formatDate(expense.expense_date)}</td>
                                        <td class="py-3 pr-4">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <Input id="pan" name="pan" bind:value={$form.pan} class="font-mono uppercase" />
                        </div>
                        <div class="space-y-2">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="flex items-center gap-3">
                            <Checkbox id="tds_applicable" name="tds_applicable" bind:checked={$form.tds_applicable} />
                            ...
                            <div class="space-y-2">
                                ...
                                <Select.Root type="single" name="tds_section" bind:value={$form.tds_section}>
                                    <Select.Trigger id="tds_section">
                                        {TDS_SECTIONS.find((s) => s.value === $form.tds_section)?.label || "Select"}
                                    </Select.Trigger>
                                    <Select.Content>
